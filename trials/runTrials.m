...
for t = 1:const.trial_per_block
    ...
    % Var 1 : image/blank size
    var1 = expDes.blockMat(t,3);
    if var1 == 1
        im_size_txt = 'large';
        dest_rect = const.large_rect;
    elseif var1 == 2
        im_size_txt = 'small';
        dest_rect = const.small_rect;
    end
    
    % Var 2 : image/blank number
    var2 = expDes.blockMat(t,4);
    
    % Load image
    if var2 < 81
        dirImage                =   sprintf('stim/im/im%i_%s.jpg',var2,im_size_txt);
    ...
    if const.checkTrial && const.expStart == 0
        fprintf(1,'\n\n\t========================  TRIAL %3.0f ========================\n',t);
        fprintf(1,'\n\tImage/blank size             =\t%s',expDes.txt_var1{var1});
        fprintf(1,'\n\tImage/blank number           =\t%s',expDes.txt_var2{var2});
    end
    ...
    while nbf <= trial_num
        ...
        if nbf > 0 && nbf <= const.freeview_num
            if var2 < 81
                Screen('DrawTexture',scr.main,t_handle,texrect,dest_rect);
            else
                Screen('FillRect',scr.main,const.dark_gray,dest_rect);
            end
        end
